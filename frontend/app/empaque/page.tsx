...
export default function ListaEmpaquePage() {
  ...
  async function cargarOrdenes() {
    ...
    setError("");
    try {
      const data = await apiFetch<OrdenEmpaque[]>("/api/empaque", { rol: "Empaque" });
      setOrdenes(data || []);
    } catch (err: any) {
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold mb-1">Órdenes listas para empaque</h1>
      ...
      {!cargando && !error && ordenes.length === 0 && (
        <p className="text-gray-500">No hay órdenes pendientes de empaque en este momento.</p>
      )}
      ...
      <div className="space-y-3">
        {ordenes.map((orden) => (
          <Link
            key={orden.id_pedido}
            href={`/empaque/${orden.id_pedido}`}
            className="block bg-white rounded-xl border border-gray-200 p-4 hover:border-teal-500 hover:shadow-sm transition-all"
          >
